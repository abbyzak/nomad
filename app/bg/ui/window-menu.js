...
import {
  createShellWindow,
  toggleShellInterface,
  getActiveWindow,
  getFocusedDevToolsHost,
} from './windows';
import {
  runSelectFileDialog,
  runForkFlow,
  runDrivePropertiesFlow,
  exportDriveToFilesystem,
    ...
    submenu: [
      {
        ...
        click: function (item) {
          if (win) {
            tabManager.create(win, undefined, {
              setActive: true,
      ...
      { type: 'separator' },
      {
        ...
        click: (item) => {
          createWindowIfNone(win, async (win) => {
            ...
            tabManager.create(win, res[0].url, {
              setActive: true,
              adjacentActive: true,
            });
          });
      ...
      // },
      {
        ...
        click: async (item) => {
          ...
          if (filePath)
            download(win, win.webContents, url, {
              saveAs: filePath,
              suppressNewDownloadEvent: true,
            });
        },
      },
      {
  ...
  var viewMenu = {
    ...
    submenu: [
      ...
      { type: 'separator' },
      {
        ...
        enabled: !noWindows,
        accelerator: `${process.platform !== 'darwin' ? 'Ctrl+Alt' : 'Ctrl+Cmd'
          }+Up`,
        click() {
          if (tab && tab.activePane) {
      ...
      { type: 'separator' },
      {
        ...
        label: 'Move Pane Up',
        enabled: !noWindows,
        accelerator: `Shift+${process.platform !== 'darwin' ? 'Ctrl+Alt' : 'Ctrl+Cmd'
          }+Up`,
        click() {
          if (tab && tab.activePane) {
    ...
    submenu: [
      ...
      { type: 'separator' },
      {
        ...
        click: async (item) => {
          ...
          let confirmation = await dialog.showMessageBox({
            type: 'question',
            message: `Import ${filePaths.length > 1 ? `${filePaths.length} files` : filePaths[0]
      ...
      },
      {
        ...
        click: async (item) => {
          ...
          let confirmation = await dialog.showMessageBox({
            type: 'question',
            message: `Import ${filePaths.length > 1
      ...
      },
      {
        ...
        click: async (item) => {
          ...
          let confirmation = await dialog.showMessageBox({
            type: 'question',
            message: `Export ${driveInfo.title || driveInfo.key} to ${filePaths[0]
    ...
    submenu: [
      {
        ...
        click: function (item) {
          if (win)
            tabManager.create(win, 'https://nomad.buddyboard.io/help', {
              setActive: true,
            });
        },
      },
