...
// Store window opacity levels
const windowOpacity = {}; // map of [window.id] => opacity (0.3 to 1.0)
...
// Function to toggle browser visibility
export function toggleBrowserVisibility(win) {
  if (win.isVisible()) {
    win.hide();
  } else {
    win.show();
...
// Function to increase window opacity
export function increaseOpacity(win) {
  if (!windowOpacity[win.id]) {
    windowOpacity[win.id] = DEFAULT_OPACITY;
  }

  windowOpacity[win.id] = Math.min(DEFAULT_OPACITY, windowOpacity[win.id] + OPACITY_STEP);
  win.setOpacity(windowOpacity[win.id]);
}

// Function to decrease window opacity
export function decreaseOpacity(win) {
  if (!windowOpacity[win.id]) {
    windowOpacity[win.id] = DEFAULT_OPACITY;
  }

  windowOpacity[win.id] = Math.max(MIN_OPACITY, windowOpacity[win.id] - OPACITY_STEP);
  win.setOpacity(windowOpacity[win.id]);
}

// Function to take screenshot and copy to clipboard
export function captureScreenshotToClipboard(win) {
  win.webContents.capturePage().then(image => {
    clipboard.writeImage(image);
    // Optional: Show a notification that the screenshot was taken
    if (win) {
