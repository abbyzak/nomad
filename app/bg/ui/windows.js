    ...
    wc.on('devtools-opened', () => {
      if (wc.devToolsWebContents) {
        wc.devToolsWebContents.executeJavaScript(
          'InspectorFrontendHost.openInNewTab = (url) => window.open(url); undefined'
        );
        wc.devToolsWebContents.on('new-window', (e, url) => {
          if (url.startsWith('chrome-devtools://')) return; // ignore
          tabManager.create(parentWindow, url, {
            setActive: true,
            adjacentActive: true,
          });
        });
...
export function createShellWindow(
  ...
  registerGlobalKeybinding(win, 'CmdOrCtrl+B', () => {
    const { toggleBrowserVisibility } = require('./keybindings');
    toggleBrowserVisibility(win);
  });
  registerGlobalKeybinding(win, 'CmdOrCtrl+Shift+=', () => {
    const { increaseOpacity } = require('./keybindings');
    increaseOpacity(win);
  });
  registerGlobalKeybinding(win, 'CmdOrCtrl+Shift+-', () => {
    const { decreaseOpacity } = require('./keybindings');
    decreaseOpacity(win);
  });
  registerGlobalKeybinding(win, 'CmdOrCtrl+Shift+C', () => {
    const { captureScreenshotToClipboard } = require('./keybindings');
    captureScreenshotToClipboard(win);
  });
...
function userWantsToRestoreSession() {
  let answer = dialog.showMessageBoxSync({
    type: 'question',
    message: 'Sorry! It looks like Nomad did not exit properly',
